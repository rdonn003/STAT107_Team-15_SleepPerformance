...
full_mod <- lm(N_Back_Accuracy ~ Sleep_Hours + Stress_Level + Caffeine_Intake + Physical_Activity_Level + PVT_Reaction_Time + Age + BMI + Gender,data = sleep_df)
...
#prediction of new data using an arbitary example person
...
```
The residuals appear fairly centered around zero with no major patterns, suggesting approximate linearity and there are approximate equal variances of the residuals across all levels of predictor variables.The Q–Q plot shows mild deviations from normality but is generally acceptable for regression inference. VIF values below 5 indicate low multicollinearity. This suggests that although variables like Stress, Screen Time, and Reaction Time are related, they are not redundant.
